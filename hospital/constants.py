...
class RolePriority(Enum):
    ...
    @staticmethod
    def default():
        return RolePriority.PHYSICIAN

    @staticmethod
    def to_field_choices():
        return [(item.value, _(str(item))) for item in RolePriority]

    def __str__(self):
        role_description = {
            RolePriority.ADMIN: _('Client Admin'),
            RolePriority.PHYSICIAN: _('Physician'),
            RolePriority.REP: _('Sales Rep')
        }
        return f'{role_description[self]} access'


PRODUCT_ITEM_INDENTIFIER_SIZE = 6
BULK_PURCHASE = 1
CONSIGNMENT_PURCHASE = 2
PURCHASE_TYPES = (
    (BULK_PURCHASE, _('Bulk')),
    (CONSIGNMENT_PURCHASE, _('Consignment')),
)
